    ...
    def setupHintImage(self, is_plant=True, from_tool = False):
        ...
        if pos and self.mouse_image:
            ...
            if is_plant and self.plant_name in {c.LILYPAD, "花盆（未实现）", c.TANGLEKLEP}:
                ...
                self.hint_rect.centerx = max(self.map.getMapGridPos(5, 0)[0], pos[0]) if self.direction else min(self.map.getMapGridPos(6, 0)[0], pos[0])
            ...
            for zombie in self.zombie_groups[i]:
                ...
                for plant in self.plant_groups[i]:
                    ...
                    if collided_func(plant, zombie):
                        ...
                        elif plant.name in  {c.SPIKEWEED,c.SPIKEROCK} and zombie.name == c.GARGANTUAR:
                            attackable_common_plants.append(plant)
